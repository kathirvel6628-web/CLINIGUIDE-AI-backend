...
try {
  ...
  if (
    ...
    console.log(
      "[TWILIO] MOCK FINAL mode - No valid keys, using logs. Perfect for hackathon."
    );
  }
} catch (e) {
  console.log(
    "[TWILIO] MOCK FINAL mode - twilio package not installed or error:",
    e.message
  );
}
...
async function sendWA(to, message) {
  ...
  try {
    ...
    console.log(
      `[REAL WA FAILED] to ${waTo} ERROR:${err.message} CODE:${err.code}`
    );
    if (err.code === 21654)
      console.log(
        "HINT: Trial needs ContentSid. Use MOCK for hackathon by deleting Twilio env vars."
      );
    if (err.code === 63007)
      console.log(
        "HINT: Recipient must send join <code> to +14155238886 first. Error 63007 = not in sandbox."
      );
    if (err.code === 21211)
      console.log("HINT: Invalid number format. Use 91 + number.");
...
function initDB() {
  return new Promise((resolve, reject) => {
    ...
    const database = new sqlite3.Database(dbPath, (err) => {
      ...
      database.serialize(() => {
        database.run(`CREATE TABLE IF NOT EXISTS patients ( id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, patient_phone TEXT, guardian1 TEXT, guardian2 TEXT, guardian3 TEXT, language TEXT DEFAULT 'en', created_at DATETIME DEFAULT CURRENT_TIMESTAMP )`);
        database.run(`CREATE TABLE IF NOT EXISTS medicines ( id INTEGER PRIMARY KEY AUTOINCREMENT, patient_id INTEGER, drug_name TEXT, dosage TEXT, raw_line TEXT, FOREIGN KEY(patient_id) REFERENCES patients(id) )`);
        database.run(`CREATE TABLE IF NOT EXISTS doses ( id INTEGER PRIMARY KEY AUTOINCREMENT, patient_id INTEGER, medicine_id INTEGER, drug_name TEXT, dosage TEXT, scheduled_date TEXT, scheduled_time TEXT, day_number INTEGER, total_days INTEGER, status TEXT DEFAULT 'pending', missed_count INTEGER DEFAULT 0, FOREIGN KEY(patient_id) REFERENCES patients(id) )`);
        // Add language column if not exists (for old DB)
        database.run(
          `ALTER TABLE patients ADD COLUMN language TEXT DEFAULT 'en'`,
          () => {}
        );
      });
      // Promisify
...
app.get("/", (req, res) =>
  res.json({
    ok: true,
    mode: twilioClient ? "REAL" : "MOCK FINAL",
    message:
      "CliniGuide AI Backend with Language Support - MOCK works for hackathon",
  })
);
...
app.get("/api/test-wa", async (req, res) => {
  ...
  const ok = await sendWA(
    to,
    "Test from CliniGuide AI - MOCK/REAL mode working! Time:" +
...
app.post("/api/register", async (req, res) => {
  try {
    const { name, patient_phone, guardian1, guardian2, guardian3, language } =
      req.body;
    if (!patient_phone || !guardian1)
      return res.json({
        ok: false,
        error: "patient_phone and guardian1 required",
      });
    await db.runAsync("DELETE FROM doses");
    await db.runAsync("DELETE FROM medicines");
    await db.runAsync("DELETE FROM patients");
    const result = await db.runAsync(
      "INSERT INTO patients (name, patient_phone, guardian1, guardian2, guardian3, language) VALUES (?,?,?,?,?,?)",
      [
        name || "",
        patient_phone,
        guardian1,
    ...
    console.log(
      `[REGISTER] Patient ${name} Lang:${language} Phone:${patient_phone} Guardian:${guardian1}`
    );
...
app.post("/api/medicines", async (req, res) => {
  try {
    ...
    if (!medicines || !medicines.length)
      return res.json({ ok: false, error: "no medicines" });
    const patient = await db.getAsync(
      "SELECT * FROM patients ORDER BY id DESC LIMIT 1"
    );
    if (!patient)
      return res.json({ ok: false, error: "no patient registered" });
    if (language) {
      await db.runAsync("UPDATE patients SET language=? WHERE id=?", [
        language,
        patient.id,
      ]);
    }
    ...
    for (let med of medicines) {
      const medRes = await db.runAsync(
        "INSERT INTO medicines (patient_id, drug_name, dosage, raw_line) VALUES (?,?,?,?)",
      ...
      for (let day = 0; day < totalDays; day++) {
        ...
        for (let t of times) {
          ...
          await db.runAsync(
            "INSERT INTO doses (patient_id, medicine_id, drug_name, dosage, scheduled_date, scheduled_time, day_number, total_days) VALUES (?,?,?,?,?,?,?,?)",
            [
              patient.id,
              medId,
              med.drug_name,
    ...
    console.log(
      `[MEDICINES] ${ medicines.length } medicines -> ${totalDoses} doses created Lang:${ language || patient.language }`
    );
    res.json({
      ok: true,
      totalDoses,
...
app.post("/api/doses/:id/taken", async (req, res) => {
  try {
    await db.runAsync("UPDATE doses SET status='taken' WHERE id=?", [
      req.params.id,
    ]);
    res.json({ ok: true });
  } catch (e) {
...
cron.schedule("* * * * *", async () => {
  try {
    ...
    for (let time in groups) {
      ...
      if (diff < 0) continue;
      const patient = await db.getAsync("SELECT * FROM patients WHERE id=?", [
        list[0].patient_id,
      ]);
      if (!patient) continue;
      ...
      if (diff >= 0 && mc === 0) {
        let msg =
          lang === "ta"
            ? `நாள் ${list[0].day_number}/${list[0].total_days} - ${time} மணிக்கு ${list.length} மருந்துகள்: ${names}`
        ...
        for (let d of list)
          await db.runAsync("UPDATE doses SET missed_count=1 WHERE id=?", [
            d.id,
          ]);
        console.log(
          `[AUTO 0min] to PATIENT ${patient.patient_phone} at ${time} Lang:${lang}`
        );
      } else if (diff >= 1 && mc === 1) {
        let msg =
          lang === "ta"
            ? `1வது நினைவூட்டல் (1 நிமிடம் தவறியது): ${names}`
            : `1st Reminder (1 min missed): ${list.length} medicines at ${time}: ${names}`;
        await sendWA(patient.patient_phone, msg);
        for (let d of list)
          await db.runAsync("UPDATE doses SET missed_count=2 WHERE id=?", [
            d.id,
          ]);
      } else if (diff >= 2 && mc === 2) {
        let msg =
          lang === "ta"
            ? `2வது நினைவூட்டல் (2 நிமிடம்): ${names}`
            : `2nd Reminder (2 mins missed): ${names}`;
        await sendWA(patient.patient_phone, msg);
        for (let d of list)
          await db.runAsync("UPDATE doses SET missed_count=3 WHERE id=?", [
            d.id,
          ]);
      } else if (diff >= 3 && mc < 4) {
        // FIXED: mc<4 guarantees guardian even if Render slept
        let msg =
          lang === "ta"
            ? `தவறியது: நோயாளி ${ patient.name || "" } ${time} மணி மருந்து தவறவிட்டார்: ${names} நாள் ${ list[0].day_number }/${list[0].total_days} Patient +${patient.patient_phone}`
            : `Missed: Patient ${patient.name || ""} missed ${ list.length } medicines at ${time}: ${names} Day ${list[0].day_number}/${ list[0].total_days } Patient +${patient.patient_phone}`;
        console.log(`[AUTO 3min] WARNING to GUARDIANS Lang:${lang}: ${msg}`);
        if (patient.guardian1) {
          await sendWA(patient.guardian1, msg);
          console.log(`-> Sent to Guardian1 ${patient.guardian1}`);
        }
        if (patient.guardian2) {
          await sendWA(patient.guardian2, msg);
          console.log(`-> Sent to Guardian2 ${patient.guardian2}`);
        }
        if (patient.guardian3) {
          await sendWA(patient.guardian3, msg);
          console.log(`-> Sent to Guardian3 ${patient.guardian3}`);
        }
        for (let d of list)
          await db.runAsync(
...
initDB()
  .then((database) => {
    ...
    app.listen(PORT, () =>
      console.log(
        `CliniGuide AI Backend Running on ${PORT} Mode:${ twilioClient ? "REAL" : "MOCK FINAL" } with Language Support`
      )
    );
  })
  .catch((err) => {
