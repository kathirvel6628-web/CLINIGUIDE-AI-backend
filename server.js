...
function initTwilio() {
  try {
    ...
    if (sid.startsWith('AC') && sid.length > 20 && token.length > 20 && wnum.includes('whatsapp:')) {
      ...
      WA_FROM = wnum;
      console.log(`[TWILIO] âœ… REAL MODE ACTIVE: ${wnum}`);
      return true;
    } else {
      console.log('[TWILIO] âš ï¸ MOCK MODE - Missing env: Need AC SID + Token + whatsapp:+1415... in TWILIO_WHATSAPP_NUMBER');
    ...
    console.log('[TWILIO] âŒ INIT ERROR:', e.message);
    return false;
  }
}
...
async function sendWA(to, message) {
  ...
  if (twilioClient && WA_FROM) {
    try {
      ...
      console.log(`[REAL WA SENT âœ…] to ${num} SID:${r.sid} | ${message.substring(0,80)}`);
      return true;
    } catch (err) {
      console.log(`[REAL WA FAILED âŒ] to ${num}: ${err.message} - Code:${err.code}`);
...
app.get('/api/test-wa', async (req, res) => {
  ...
  const ok = await sendWA(to, `âœ… CliniGuide REAL Test OK ${new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" })} IST - Day 1/7 Time for 1 medicines at ${new Date().toLocaleTimeString()} - If you get this, REAL mode works!`);
  res.json({
    ok,
    mode: twilioClient ? 'REAL' : 'MOCK FIXED',
    to: to,
...
app.post('/api/sos', async (req, res) => {
  try {
    ...
    const msg = `ðŸš¨ SOS! Patient ${p.name || 'Patient'} needs help! Phone: ${p.patient_phone} Time:${new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" })}`;
    console.log(`[SOS] ${msg}`);
...
cron.schedule('* * * * *', async () => {
  try {
    ...
    for (let time in groups) {
      ...
      // 0 min - Patient first
      if (diff >= 0 && mc === 0) {
        const msg = `ðŸ’Š Day ${list[0].day_number}/${list[0].total_days} Time for ${list.length} medicines at ${displayTime}: ${names}. Reply TAKEN or click button in app.`;
      ...
      // 1 min - Patient reminder 2
        ...
        const msg = `â° Reminder Day ${list[0].day_number}/${list[0].total_days} - Still pending ${list.length} medicines at ${displayTime}: ${names}. Please take now.`;
        await sendWA(patient.patient_phone, msg);
      ...
      // 2 min - Patient last reminder
        ...
        const msg = `âš ï¸ Last Reminder Day ${list[0].day_number}/${list[0].total_days} - ${list.length} medicines at ${displayTime}: ${names} - Take now or guardian will be alerted.`;
        await sendWA(patient.patient_phone, msg);
      ...
      // 3 min - Guardian warning - REAL TIME USABLE
        ...
        const msg = `ðŸš¨ Missed Alert: Patient ${patient.name || 'Patient'} missed ${list.length} medicines at ${displayTime}: ${names}. Phone: ${patient.patient_phone}. Please check immediately. Day ${list[0].day_number}/${list[0].total_days}`;
...
app.listen(PORT, () => console.log(`âœ… REAL-READY Backend Running on ${PORT} Mode:${twilioClient ? 'REAL AUTO SENDING' : 'MOCK - Add Twilio env to make REAL'} - IST:${new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" })}`));
